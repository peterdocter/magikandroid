...
/**
 * Layout container for a view hierarchy that can be scrolled by the user, allowing it to be larger than the physical display. A ScrollView is a {@link FrameLayout}, meaning
 * you should place one child in it containing the entire contents to scroll; this child may itself be a layout manager with a complex hierarchy of objects. A child that is
 * often used is a {@link LinearLayout} in a vertical orientation, presenting a vertical array of top-level items that the user can scroll through.
 * 
 * <p>
 * You should never use a ScrollView with a {@link ListView}, since ListView takes care of its own scrolling. Most importantly, doing this defeats all of the important
 * optimizations in ListView for dealing with large lists, since it effectively forces the ListView to display its entire list of items to fill up the infinite container
 ...
 * view within a larger container.
 * 
 * <p>
 * FullScrollView supports vertical and horizontal scrolling. FullScrollView is based on ScrollView and extended according to symmetry so horizontal scrolling would be
 * possible. Contains many bugs and documentation has not been updated, but kinda works when containing one TextView as a child. pjv 2009-01-24
 */
public class FullScrollView extends FrameLayout
{
    ...
    /**
     * Flag to indicate that we are moving focus ourselves. This is so the code that watches for focus changes initiated outside this ScrollView knows that it does not have to
     * do anything.
     */
    ...
    /**
     * The child to give focus to in the event that a child has requested focus while the layout is dirty. This prevents the scroll from being wrong if the child has not been
     * laid out before requesting focus.
     */
    private View mChildToScrollTo = null;

    /**
     * True if the user is currently dragging this ScrollView around. This is not the same as 'is being flinged', which can be checked by mScroller.isFinished() (flinging
...
//        data.crearFile( CLASE_DYSPLAY , "TIME;Velocity" );
        ...
        data.writeToFile( rtaY, true, CLASE_DYSPLAY );
        rtaX = "\n" + "X" + "\n";
        rtaY = "\n" + "Y" + "\n";
    }
    ...
    public boolean onInterceptTouchEvent( MotionEvent ev )
    {
        ...
        // String captura = "";
        // captura = display.velocityTrack( ev );
        // if( captura != "" )
        // {
        // rta += "\n";
        // rta += captura;
        ...
        switch( action )
        {
            case MotionEvent.ACTION_MOVE:
                ...
                /*
                 * Locally do absolute value. mLastMotionY is set to the x value of the down event.
                 */
    ...
    public boolean onTouchEvent( MotionEvent ev )
    {
        ...
        switch( action )
        {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if( localLOGV )
                    Log.v( TAG, "onTouchEvent ActionMove deltaX=" + deltaX + " deltaY=" + deltaY );

                if( deltaX < 0 )
                {
                    if( mScrollX >= 0 )
                    {// FIX Changed, don't know why, pjv.
                        scrollBy( deltaX, 0 );
                    }
                }
                else if( deltaX > 0 )
     ...
     * </p>
     * 
     * @param leftFocus look for a candidate is the one at the left of the bounds if leftFocus is true, or at the bottom of the bounds if leftFocus is false
     ...
     */
    private View findFocusableViewInMyBoundsX( final boolean leftFocus, final int left, View preferredFocusable )
    {
        /*
        ...
        return findFocusableViewInBoundsX( leftFocus, leftWithoutFadingEdge, rightWithoutFadingEdge );
    }
    /**
     * <p>
     ...
     * </p>
     * 
     * @param topFocus look for a candidate is the one at the top of the bounds if topFocus is true, or at the bottom of the bounds if topFocus is false
     ...
     * @return the next focusable component in the bounds or null if none can be found
     */
    private View findFocusableViewInMyBoundsY( final boolean topFocus, final int top, View preferredFocusable )
    {
        /*
         * The fading edge's transparent side should be considered for focus since it's mostly visible, so we divide the actual fading edge length by 2.
         */
        ...
        return findFocusableViewInBoundsY( topFocus, topWithoutFadingEdge, bottomWithoutFadingEdge );
    }
     ...
     * </p>
     * 
     * @param leftFocus look for a candidate is the one at the left of the bounds if leftFocus is true, or at the right of the bounds if leftFocus is false
    ...
    private View findFocusableViewInBoundsX( boolean leftFocus, int left, int right )
    {
        ...
        /*
         * A fully contained focusable is one where its left is right of the bound's left, and its right is left of the bound's right. A partially contained focusable is one
         * where some part of it is within the bounds, but it also has some part that is not within bounds. A fully contained focusable is preferred to a partially contained
         * focusable.
         */
    ...
    private View findFocusableViewInBoundsY( boolean topFocus, int top, int bottom )
    {
        ...
        /*
         * A fully contained focusable is one where its top is below the bound's top, and its bottom is above the bound's bottom. A partially contained focusable is one where
         * some part of it is within the bounds, but it also has some part that is not within bounds. A fully contained focusable is preferred to a partially contained
         * focusable.
         */
        boolean foundFullyContainedFocusable = false;
    ...
    /**
     * <p>
     * Handles scrolling in response to a "page up/down" shortcut press. This method will scroll the view by one page up or down and give the focus to the topmost/bottommost
     * component in the new visible area. If no component is a good candidate for focus, this scrollview reclaims the focus.
     * </p>
     * 
     * @param direction the scroll direction: {@link android.view.View#FOCUS_UP} to go one page up or {@link android.view.View#FOCUS_DOWN} to go one page down
    ...
    /**
     * <p>
     * Handles scrolling in response to a "home/end" shortcut press. This method will scroll the view to the top or bottom and give the focus to the topmost/bottommost
     * component in the new visible area. If no component is a good candidate for focus, this scrollview reclaims the focus.
     * </p>
     * 
    ...
    /**
     * <p>
     * Scrolls the view to make the area defined by <code>top</code> and <code>bottom</code> visible. This method attempts to give the focus to a component visible in this
     * area. If no component can be focused in the new visible area, the focus is reclaimed by this scrollview.
     * </p>
     * 
    ...
    public boolean arrowScroll( int direction )
    {
        ...
        if( nextFocused != null && isWithinDeltaOfScreenX( nextFocused, maxJumpX ) && isWithinDeltaOfScreenY( nextFocused, maxJumpY ) )
        {// TODO
    ...
    public final void smoothScrollBy( int dx, int dy )
    {
        ...
        {
            if( localLOGV )
                Log.v( TAG, "Smooth scroll: mScrollX=" + mScrollX + " dx=" + dx + "mScrollY=" + mScrollY + " dy=" + dy );
        ...
        else
        {
            ...
            if( localLOGV )
                Log.v( TAG, "Immediate scroll: mScrollX=" + mScrollX + " dx=" + dx + "mScrollY=" + mScrollY + " dy=" + dy );
            scrollBy( dx, dy );
        }
    ...
    public void computeScroll( )
    {
        if( mScroller.computeScrollOffset( ) )
        {
            ...
            if( getChildCount( ) > 0 )
            {
                ...
                if( localLOGV )
                    Log.v( TAG, "mScrollX=" + mScrollX + " x=" + x + " width=" + this.getWidth( ) + " child width=" + child.getWidth( ) + " mScrollY=" + mScrollY + " y=" + y + " height=" + this.getHeight( ) + " child height=" + child.getHeight( ) );
            }
            else
            {
                mScrollX = x;
    ...
    /**
     * Compute the amount to scroll in the X direction in order to get a rectangle completely on the screen (or, if taller than the screen, at least the first screen size
     * chunk of it).
     * 
     * @param rect The rect.
    ...
    protected int computeScrollDeltaToGetChildRectOnScreenX( Rect rect )
    {
        ...
        if( localLOGV )
            Log.v( TAG, "child=" + rect.toString( ) + " screenLeft=" + screenLeft + " screenRight=" + screenRight + " width=" + width );
        if( rect.right > screenRight && rect.left > screenLeft )
        {
            ...
            if( localLOGV )
                Log.v( TAG, "scrollXDelta=" + scrollXDelta + " distanceToRight=" + distanceToRight );
    ...
    /**
     * Compute the amount to scroll in the Y direction in order to get a rectangle completely on the screen (or, if taller than the screen, at least the first screen size
     * chunk of it).
     * 
     * @param rect The rect.
     * @return The scroll delta.
    ...
    protected int computeScrollDeltaToGetChildRectOnScreenY( Rect rect )
    {
        ...
        if( localLOGV )
            Log.v( TAG, "child=" + rect.toString( ) + " screenTop=" + screenTop + " screenBottom=" + screenBottom + " height=" + height );
        if( rect.bottom > screenBottom && rect.top > screenTop )
        {
            ...
            if( localLOGV )
                Log.v( TAG, "scrollYDelta=" + scrollYDelta + " distanceToBottom=" + distanceToBottom );
     ...
     * 
     * @param velocityX The initial velocity in the X direction. Positive numbers mean that the finger/cursor is moving right the screen, which means we want to scroll towards
     *        the left.
     */
    public void flingX( int velocityX )
    {
        ...
        View newFocused = findFocusableViewInMyBoundsX( movingRight, mScroller.getFinalX( ), findFocus( ) );
        if( newFocused == null )
        {
            newFocused = this;
        }

        if( newFocused != findFocus( ) && newFocused.requestFocus( movingRight ? View.FOCUS_DOWN : View.FOCUS_UP ) )
        {
            mScrollViewMovedFocus = true;
            mScrollViewMovedFocus = false;
        }
     ...
     * Fling the scroll view
     * 
     * @param velocityY The initial velocity in the Y direction. Positive numbers mean that the finger/cursor is moving down the screen, which means we want to scroll towards
     *        the top.
     */
    public void flingY( int velocityY )
    {
        ...
        View newFocused = findFocusableViewInMyBoundsY( movingDown, mScroller.getFinalY( ), findFocus( ) );
        if( newFocused == null )
        {
            newFocused = this;
        }
    ...
    private int clamp( int n, int my, int child )
    {
        ...
        {
            /*
             * my >= child is this case: |--------------- me ---------------| |------ child ------| or |--------------- me ---------------| |------ child ------| or
             * |--------------- me ---------------| |------ child ------|
             * 
